...
class lvisData(object):
    ...
    def __init__(self, filename, setElev=False, minX=-100000000, maxX=100000000, minY=-1000000000, maxY=100000000, onlyBounds=False):
        ...
        '''
        # call the file reader and load into the self
    ...
    def readLVIS(self, filename, minX, minY, maxX, maxY, onlyBounds):
        ...
        lon0 = f['LON0'][:]  # longitude of waveform top / changed from sc
        lat0 = f['LAT0'][:]  # latitude of waveform top / changed from sc
        lonN = f['LON' + str(self.nBins - 1)][:]  # longitude of waveform bottom / changed from sc
        latN = f['LAT' + str(self.nBins - 1)][:]  # latitude of waveform bottom / changed from sc
